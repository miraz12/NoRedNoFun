...
public:
  ...
  void actionMove(void (*f)(unsigned int key, BotInterface *bot)) override {
    moveFunc = f;
  };
  void actionFire(void (*f)(BotInterface *bot)) override { fireFunc = f; };
  void actionLookAt(void (*f)(int x, int y, BotInterface *bot)) override {
    lookFunc = f;
  };
  void actionLookInDirection(void (*f)(float x, float y,
                                       BotInterface *bot)) override {
    lookInDirectionFunc = f;
  };
...
#ifdef _WIN32
extern "C" __declspec(dllexport) BotInterface *newInterface(unsigned int id) {
